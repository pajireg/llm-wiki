...
4. **Apply changes**:
   ...
   - **`aliases` + `keywords`**: on every created/updated wiki page, write search-expansion frontmatter so concept/synonym queries can find it:
     - `aliases:` — alternate names, abbreviations, and the term in the user's primary language **and** English (e.g. for a Korean user writing about Kubernetes: `["쿠버네티스", "Kubernetes", "K8s"]`). High-precision.
